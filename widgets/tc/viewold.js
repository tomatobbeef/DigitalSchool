...
//     }
    ...
    function drawAndFlatten() {
        ...
        function finishDrawing() {
            if (polygonPos.length < 3) {
                console.log("至少需要三个点来绘制多边形");
                return;
            }
    
            if (polygon) {
                polygon.polygon.hierarchy = new Cesium.PolygonHierarchy(polygonPos);
                polygon.polygon.material = Cesium.Color.fromCssColorString('#a8e362').withAlpha(0.2);
                flattenModels();
            }
            isDrawing = false;
        }
        ...
        // 判断一个点是否在多边形内
        function isPointInPolygon(point, polygonPos) {
            var x = point.x, z = point.z;
            var inside = false;
            for (var i = 0, j = polygonPos.length - 1; i < polygonPos.length; j = i++) {
                var xi = polygonPos[i].x, zi = polygonPos[i].z;
                var xj = polygonPos[j].x, zj = polygonPos[j].z;
        ...
        function flattenModels() {
            var minY = Math.min(...polygonPos.map(pos => pos.y));
    
            tileset.tileLoad.addEventListener(function(tile) {
                tile.contentReadyPromise.then(function(content) {
                    var features = content.featuresLength;
                    for (var i = 0; i < features; i++) {
                        var feature = content.getFeature(i);
                        var positions = feature.getGeometryInstanceAttributes().positions;
    
                        for (var j = 0; j < positions.length; j += 3) {
                            var position = new Cesium.Cartesian3(positions[j], positions[j+1], positions[j+2]);
                            if (isPointInPolygon(position, polygonPos)) {
                                positions[j+1] = minY; // 压平到最低点
                            }
                        }
                    }
                });
