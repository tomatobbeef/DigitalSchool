...
async function show3dgs(modelname) {
  let carousel = true;
  ...
  const url = new URL(
    // "nike.splat",
    // location.href,
    params.get("url") || modelname,
    "http://localhost:5173/src/assets/model/"
  );
  ...
  canvas.addEventListener("mousemove", e => {
    ...
    if (down == 1) {
      ...
      if (Math.abs(dx) > Math.abs(dy)) {
        // 如果X方向移动距离大于Y方向，则只围绕Z轴旋转
        inv = rotate4(inv, dx, 0, 1, 0);
      } else {
        // 反之，如果Y方向移动距离大于或等于X方向，则只围绕X轴旋转
